...
async function getInputValuesAndExecuteSearch() {
  ...
  const owApiKey = '7e39e44542afffe07cf335a084d1a09f';
  let owApiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${searchString}&appid=${owApiKey}&units=metric`;

  const fourSquareClientId = '1LGQ3HLEBPTDLDX0YFON0ENXFPNADCUZMB1FHPK0IDBDELDT'
  const fourSquareClientSecret = '5Y13EYEUCHUV50BZZE0F5UKNYWBDD25QLFJ4COIS0UOAMH1N'
  let fourSquareUrl = `https://api.foursquare.com/v2/venues/explore?near=${searchString}&v=${todayYYYYMMDD}&client_id=${fourSquareClientId}&client_secret=${fourSquareClientSecret}&limit=9`
...
class result {
  constructor(index, name, address, city, iconPlaceUrl) {
    this.index = index;
    this.name = name;
